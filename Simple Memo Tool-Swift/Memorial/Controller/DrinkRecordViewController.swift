...
class DrinkRecordViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(loadData), name: NSNotification.Name(rawValue: MT_Drink_NTF), object: nil)
    ...
    func loadModels() -> [MemoModel]? {
        return NSKeyedUnarchiver.unarchiveObject(withFile: MemoModel.DrinkArchiveURL.path) as? [MemoModel]
    }
    ...
    func saveModel() {
        let isSuccessfulSave = NSKeyedArchiver.archiveRootObject(dataArr, toFile: MemoModel.DrinkArchiveURL.path)
        if !isSuccessfulSave {
